...
fun LiveTryOnScreen(
    ...
    Surface(modifier = Modifier.fillMaxSize(), color = Color.Black) {
        if (granted) {
            LiveCameraContent(viewModel.pipeline, onBack)
        } else {
            PermissionContent(onBack) { launcher.launch(Manifest.permission.CAMERA) }
        }
    }
}
...
private fun LiveCameraContent(
    ...
    DisposableEffect(lifecycleOwner) {
        ...
        val analyzer = LiveFrameAnalyzer(
            pipeline = pipeline,
        ...
        Column(
            ...
            Row(verticalAlignment = Alignment.CenterVertically) {
                ...
                Column {
                    ...
                    Text("Mova sua mão devagar para testar o acompanhamento.", color = Color.White.copy(alpha = 0.85f))
                }
            }
        }
        Surface(
        ...
        try {
            ...
            if (bitmap != null) {
                ...
                val result = pipeline.process(
                    image = oriented,
                    polishColor = Color(0xFFC04A67),
                    stabilize = true,
                )
                result?.bitmap?.let { painted ->
                    if (painted !== oriented && !painted.isRecycled) {
                        oriented.recycle()
                    }
                    onResult(painted)
                } ?: run {
