...
object NailPlateCalibration {
    ...
    fun scalesFor(finger: Finger): FingerScale = when (finger) {
        Finger.THUMB -> FingerScale(0.82f, THUMB_LENGTH_SCALE)
        Finger.INDEX -> FingerScale(0.68f, 0.80f)
        Finger.MIDDLE -> FingerScale(0.70f, 0.82f)
        Finger.RING -> FingerScale(0.68f, 0.80f)
        Finger.PINKY -> FingerScale(0.66f, 0.76f)
    }
    ...
    fun isUsablePlate(plate: PlateGeometry): Boolean {
        ...
        val minAxis = when { plate.thumbMode -> MIN_AXIS_THUMB_PX; plate.facing -> MIN_AXIS_FACING_PX; else -> MIN_AXIS_OPEN_PX }
    ...
    fun plateFromPixels(finger: Finger, tipX: Float, tipY: Float, dipX: Float, dipY: Float, pipX: Float, pipY: Float, mcpX: Float, mcpY: Float): PlateGeometry {
        ...
        when {
            thumbMode -> { axisStartX = mcpX; axisStartY = mcpY; rawAxisLength = tipMcp }
            facing -> { axisStartX = pipX; axisStartY = pipY; rawAxisLength = tipPip }
            else -> {
        ...
        val widthPx = if (facing) (tipPip * FACING_WIDTH_SCALE).coerceIn(MIN_NAIL_WID_PX, MAX_NAIL_WID_PX)
        ...
        val ux = dirX / dirLen
        val uy = dirY / dirLen
        val overshootBase = when { thumbMode -> tipMcp; facing -> tipPip; else -> rawAxisLength }
        ...
        return PlateGeometry(
            centerX, centerY, lengthPx, widthPx,
            Math.toDegrees(atan2(dirX.toDouble(), -dirY.toDouble())).toFloat(),
            axisStartX, axisStartY, tipX, tipY, ux, uy, overshootPx, thumbMode, facing, rawLengthPx,
        )
    }
    ...
    fun almondExtents(plate: PlateGeometry): AlmondExtents {
        ...
        return AlmondExtents(
            tipX, tipY, cuticleX, cuticleY, midX, midY,
            halfW * TIP_WIDTH_FACTOR, halfW * midFactor, halfW * CUTICLE_WIDTH_FACTOR,
            px, py, tipPoint,
        )
    }
}
