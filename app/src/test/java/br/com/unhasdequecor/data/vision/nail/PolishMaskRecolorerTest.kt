...
class PolishMaskRecolorerTest {
    ...
    @Test
    fun `recolor returns null for non-positive source size`() {
        val source = mockk<Bitmap>()
        val mask = mockk<Bitmap>(relaxed = true)
        every { source.width } returns 0
        every { source.height } returns 10
        assertThat(PolishMaskRecolorer.recolor(source, mask, Color.Red)).isNull()
    }

    @Test
    fun `recolor paints nail region and keeps uncovered pixels`() {
        val width = 10
        val height = 10
        val source = mockk<Bitmap>()
        val out = mockk<Bitmap>()
        val mask = mockk<Bitmap>()
        every { source.width } returns width
        every { source.height } returns height
        every { source.copy(any(), any()) } returns out
        every { out.width } returns width
        every { out.height } returns height
        every { out.isRecycled } returns false
        every { mask.width } returns width
        every { mask.height } returns height
        every { out.getPixels(any(), any(), any(), any(), any(), any(), any()) } answers {
            firstArg<IntArray>().fill(PolishMaskRecolorer.packArgb(255, 110, 100, 90))
        }
        every { mask.getPixels(any(), any(), any(), any(), any(), any(), any()) } answers {
            val buf = firstArg<IntArray>()
            buf.fill(PolishMaskRecolorer.packArgb(255, 0, 0, 0))
            // ~10% coverage (abaixo do teto 18%)
            for (i in 0 until 10) {
                buf[i] = PolishMaskRecolorer.packArgb(255, 255, 255, 255)
            }
        }
        every { out.setPixels(any(), any(), any(), any(), any(), any(), any()) } just runs

        val result = PolishMaskRecolorer.recolor(source, mask, Color(0xFFC82850))
        assertThat(result).isSameInstanceAs(out)
        verify(exactly = 1) { out.setPixels(any(), any(), any(), any(), any(), any(), any()) }
        verify(exactly = 0) { out.recycle() }
    }

    @Test
    fun `recolor recycles output when mask is invalid`() {
        val width = 8
        val height = 8
        val source = mockk<Bitmap>()
        val out = mockk<Bitmap>()
        ...
        every { mask.height } returns height
        every { out.getPixels(any(), any(), any(), any(), any(), any(), any()) } answers {
            firstArg<IntArray>().fill(PolishMaskRecolorer.packArgb(255, 90, 90, 90))
        }
        every { mask.getPixels(any(), any(), any(), any(), any(), any(), any()) } answers {
            firstArg<IntArray>().fill(PolishMaskRecolorer.packArgb(255, 0, 0, 0))
        }
        every { out.recycle() } just runs
    ...
    @Test
    fun `scaledMaskOrSelf returns same instance when size matches`() {
        val mask = mockk<Bitmap>()
        every { mask.width } returns 12
        every { mask.height } returns 8
        assertThat(PolishMaskRecolorer.scaledMaskOrSelf(mask, 12, 8)).isSameInstanceAs(mask)
    }

    @Test
    fun `recycleIfScaled only recycles when mask was scaled`() {
        val original = mockk<Bitmap>(relaxed = true)
        val scaled = mockk<Bitmap>(relaxed = true)
        PolishMaskRecolorer.recycleIfScaled(original, original)
        verify(exactly = 0) { original.recycle() }
        PolishMaskRecolorer.recycleIfScaled(scaled, original)
        verify(exactly = 1) { scaled.recycle() }
    }

    @Test
    fun `loadMask returns null when asset is missing`() {
        val assets = mockk<AssetManager>()
        val context = mockk<Context>()
        every { context.assets } returns assets
        every { assets.open(any()) } throws java.io.FileNotFoundException("missing")
        assertThat(PolishMaskRecolorer.loadMask(context, "nope")).isNull()
    }
}
