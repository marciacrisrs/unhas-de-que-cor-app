...
sonar {
    properties {
        ...
        property("sonar.projectName", envOrProp("SONAR_PROJECT_NAME", "sonar.projectName") ?: "Unhas de Que Cor")
...
tasks.register("verifyCi") {
    group = "verification"
    description = "Roda o mesmo conjunto de verificações do CI (detekt, lint, unit tests, cobertura domain/app, assembleDebug + release)."
    dependsOn(":app:detekt", ":app:lintDebug", ":app:testDebugUnitTest", ":app:jacocoDomainCoverageVerification", ":app:jacocoAppCoverageVerification", ":app:assembleDebug", ":app:assembleRelease")
}
